...
> 调研执行方法论（Plan → Do → Check → Act 全流程），用于回答「这个课题该怎么查」「怎么保证调研不是拍脑袋」「多个来源冲突信谁」这类问题

SynomosAI 四支柱体系：**身份（Identity）· 溯源（Traceability）· 治理（Governance）· 共生（Symbiosis）**——当 AI 进入商业，可信是唯一的硬通货。

## 仓库内容

本仓库为 `research-methodology` 技能的发布包：核心文件 `SKILL.md` 遵循 Agent Skills 规范（YAML frontmatter），可直接放入主流 AI Agent 的技能目录使用。

- **分类**：调研方法论
- **版本**：1.0.0
- **署名**：诺源(Arche)@SynomosAI
- **许可**：MIT（详见仓库 LICENSE）
